...
class LoomiClient:
    ...
    def run(self):
        if not self.token:
            print("❌ 错误: 未配置 LOOMI_TOKEN。请从浏览器抓包获取。")
            return
        ...
        try:
            ...
            if user_resp.status_code != 200:
                self.error_exit("Token 已过期或无效，请重新抓包更新 LOOMI_TOKEN")
                return
            ...
            if credit_resp.status_code == 200:
                ...
                if data: balance = data[0].get('credits', 0)
    ...
    def error_exit(self, msg):
        print(f"❌ {msg}")
        LoomiNotifier.send_telegram("Loomi 脚本警告", f"⚠️ {msg}")
